...
# Loading the data
df <- read.csv("industrial_ind.csv")
...
industry_monthly <- industry %>%
  ...
  mutate(
    ma_3m_current = rollmean(Ind_value, k = 3, fill = NA, align = "right"),
    ma_3m_lag = lag(ma_3m_current, 3),
    growth_q = log(ma_3m_current / ma_3m_lag) * 100
  )

# Writing the prepared data set
to_write <- industry_monthly %>%
  mutate(growth_q_ind = growth_q) %>%
  select(iso_year, iso_week, growth_q_ind)

write.csv(to_write, "Industry_changes_adjusted.csv", row.names = FALSE)
