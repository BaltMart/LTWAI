...
# Granger tests
gtest_df <- quarter_ends %>%
  mutate(LTWAI_lag1 = lag(LTWAI),
         LTWAI_lag2 = lag(LTWAI_lag1),
         LTWAI_lag3 = lag(LTWAI_lag2))

grangertest(Pct_change_GDP ~ LTWAI, order = 1, data = gtest_df)
grangertest(Pct_change_GDP ~ LTWAI, order = 2, data = gtest_df)
grangertest(Pct_change_GDP ~ LTWAI, order = 3, data = gtest_df)
grangertest(Pct_change_GDP ~ LTWAI, order = 4, data = gtest_df)

# Writing the df
write.csv(df, "df_LTWAI.csv", row.names = FALSE)
